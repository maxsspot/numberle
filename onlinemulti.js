...
function checkForDisallowed () {
  ...
  if (username.value.includes ("<") || username.value.includes (">") || username.value.toLowerCase().includes("kay y ess") || username.value.toLowerCase().includes("kay why ess") || username.value.toLowerCase().includes("kys") || username.value.toLowerCase().includes("ky$") || checkedValue.includes("*")) {
...
function startGame() {
  ...
  var countdown = setInterval(function() {
    ...
    if (toStart == 0) {
      ...
      onValue(roomRef, (snapshot) => {
        const roomData = snapshot.val();
        if (snapshot.exists() && !roomData.numberToGuess) {
